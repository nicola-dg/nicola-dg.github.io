        ...
        <section id="servizi" className="pb-42 scroll-mt-20">
            <div className="flex flex-col gap-4 items-center">
                ...
                <div className="relative flex flex-col items-center ">
                    ...
                    <div className="absolute bottom-0 left-0 translate-y-2/3 -translate-x-1/3 hidden md:block">
...
const ImageGallery = () => {
    ...
    return (
        <>
            <Carousel className="w-full max-w-xs md:max-w-md lg:max-w-full" setApi={setApi}>
                <CarouselContent>
            ...
            <Dots api={api} count={count} current={current}></Dots>

        </>
    )
}

const Dots = ({ api, count, current }: { api: CarouselApi, count: number, current: number }) => {
    return (
        < div className="flex justify-center mt-4 space-x-2 md:hidden" >
            {
                Array.from({ length: count }).map((_, index) => (
                    <button
                        ...
                        className={`w-3 h-3 rounded-full transition-colors ${current === index ? "bg-black" : "bg-gray-400"
                            }`}
                    />
                ))
            }
